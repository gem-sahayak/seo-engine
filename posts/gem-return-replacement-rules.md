...
keyTakeaways:
  - "Buyer ke paas delivery ke baad product inspect karne aur reject karne ke liye standard 10 days ka return window hota hai."
  - "Product Mismatch ya specification deviation hone par entire return aur replacement logistics cost seller ko hi bear karni padti hai."
  - "Rejected material ko seller ko 10 days ke andar buyer ke premises se lift back (collect) karna hota hai, varna warehousing charges ya disposal risk face karna padta hai."
faqs:
  - q: "GeM portal par product return window kitne dino ka hota hai?"
    a: "GeM GTC ke tehat, delivery ke baad buyer ke paas product inspection aur rejection ke liye standard 10 calendar days ka time hota hai, jab tak CRAC generate na ho."
  - q: "Agar delivered product specifications se match nahi karta toh kya hota hai?"
    a: "Ise Product Mismatch mana jata hai. Buyer consignment ko reject kar sakta hai aur Seller ko apney expense par 15 days ke andar correct replacement supply karna hota hai."
  - q: "Return shipping ya transport charges kaun pay karta hai?"
    a: "Product rejection ya mismatch ke case me return logistics aur replacement supply ke sabhi charges (transport, loading, handling) Seller ko hi pay karne hote hain."
relatedTools:
  - "gem-incident-decoder"
  - "return-cost-calculator"
relatedArticles:
  - "gem-warranty-claim-process"
  - "gem-catalog-rejected-reasons"
  - "gem-order-cancel-rules"
related_entities: ['Return Window', 'Product Mismatch', 'Replacement Cost', 'Buyer Return']
related_articles: ['gem-warranty-claim-process', 'gem-catalog-rejected-reasons', 'gem-order-cancel-rules']
user_journey_stage: 'Order'
intent: 'Guide'
difficulty: 'Intermediate'
---

# GeM Return & Replacement Rules: Timelines, Product Mismatch & Shipping Cost
...
## ⚡ Quick Answer: GeM Return aur Replacement Rule Kya Hai?
...
| Rule Parameter | GTC Standard Rule & Timeline | Responsibility & Penalty Action |
| :--- | :--- | :--- |
| **Inspection & Return Window** | **10 Calendar Days** from delivery receipt | Buyer must inspect and generate rejection before CRAC. |
| **Product Replacement Timeline** | **15 Days** (or as per ATC/STC terms) | Seller must supply contract-compliant goods. |
| **Lifting of Rejected Consignment** | **10 Days** from rejection notification | Seller must collect the rejected items from buyer's site. |
| **Logistics & Return Shipping Cost** | **100% Seller Borne** | Transportation, handling, and insurance paid by Seller. |
| **CRAC Auto-Generation** | **10th Day** of delivery (if no action taken) | System auto-accepts goods; payment cycle is triggered. |
| **Failure to Lift or Replace** | Escales to GeM Incident Management | Forfeiture of ePBG, payment hold, and seller suspension. |

---

## 📌 Table of Contents
1. [Introduction to GeM Return & Replacement Framework](#1-introduction-to-gem-return--replacement-framework)
2. [Who Should Read This Guide](#2-who-should-read-this-guide)
3. [Prerequisites for Managing Returns & Replacements](#3-prerequisites-for-managing-returns--replacements)
4. [Required Documents Checklist for Rejection & Replacement](#4-required-documents-checklist-for-rejection--replacement)
5. [Eligibility Criteria: Valid Grounds for Buyer Rejections](#5-eligibility-criteria-valid-grounds-for-buyer-rejections)
6. [Step-by-Step Guide: How Return & Replacement Process Works on GeM](#6-step-by-step-guide-how-return--replacement-process-works-on-gem)
7. [Decision Tree: How to Handle a Product Rejection/Return on GeM](#7-decision-tree-how-to-handle-a-product-rejectionreturn-on-gem)
8. [Warning & Compliance Advisory (Sellers Beware!)](#8-warning--compliance-advisory-sellers-beware)
9. [Comparison: Return/Rejection (Pre-CRAC) vs. Warranty Claim (Post-CRAC)](#9-comparison-returnrejection-pre-crac-vs-warranty-claim-post-crac)
10. [AI Search / RAG Target Block](#10-ai-search--rag-target-block)
...
16. [Platform Disclaimer](#16-platform-disclaimer)
...
1. **Active GeM Seller Account Dashboard:** Incident management desk aur order execution panel ka full access.
2. **Order Dispatch Documentation:** Delivery Challan, Lorry Receipt (LR) ya Waybill, and original invoice copy.
3. **Consignee Contact Details:** Local consignee phone number aur delivery location coordinates taaki reverse pickup schedule ho sake.
4. **Dispute Resolution Evidence:** Packaging videos, NABL certified test reports ya visual proofs jo product ki genuineness prove kar sakein (in case of wrong rejection).
...
| Rejection Ground | Description & Severity | Verification Method | Action for Seller |
| :--- | :--- | :--- | :--- |
| **Product Mismatch** | Catalog specs ya contract parameters se brand/model mismatch hona. *(Severity: High)* | Physical match check against contract specs. | Replace with correct model immediately. |
| **Transit Damage** | Physical damage like cracks, broken components, water damage. *(Severity: High)* | Visual verification (photos/videos at unloading). | Replace the damaged quantity at own cost. |
| **Specification Deviation** | Material performance test me failure (e.g., lower capacity, speed, or grade). *(Severity: Extreme)* | Lab test reports (NABL/Government approved). | Submit compliant test reports or change batch. |
| **Quantity Shortage** | Delivered quantity is less than the invoiced or ordered quantity. *(Severity: Medium)* | Verification of packing boxes and weight tags. | Supply the short quantity immediately. |
| **Expiry Exceeded** | Products having less shelf-life than mentioned in ATC/STC. *(Severity: Critical)* | Printing date check on the retail box. | Replace with fresh batch items. |
...
## 8. Warning & Compliance Advisory (Sellers Beware!)
...
| Comparison Feature | Pre-CRAC Rejection (Return/Replacement) | Post-CRAC Warranty Claim |
| :--- | :--- | :--- |
| **Applicable Stage** | Delivery stage par, before acceptance certificate (CRAC) generation. | Delivery acceptance (CRAC generated) ke baad, under warranty period. |
| **Trigger Cause** | Product Mismatch, physical transit damage, specification deviation, quantity shortage. | Functional defects, performance failures, component breakdown under warranty. |
| **Logistics Cost** | **100% Seller borne**. Seller must lift back from buyer site. | Usually seller borne, but governed by standard manufacturer warranty policies (carry-in/on-site). |
| **System Impact** | Payment is blocked. CRAC is not generated for rejected quantity. | Payment is released for delivery. Incident is raised separately for warranty non-compliance if seller refuses repair. |
| **Action Window** | **10 Days** from delivery receipt for rejection; **15 Days** for replacement. | Active throughout the warranty period (e.g., 1 to 5 years). |
| **Dispute Resolution** | Handled through direct replacement or immediate Incident ticket. | Handled via service centre mapping, AMC/Warranty call logs, or portal warranty claims. |
...
* [GeM Warranty Claim Process: Step-by-Step Resolution Guide](https://sahayakai.co.in/knowledge/order-management/gem-warranty-claim-process)
* [Understanding GeM Catalog Rejection Codes and dashboard fixes](https://sahayakai.co.in/knowledge/catalog-management/gem-catalog-rejected-reasons)
* [GeM Order Cancellation Rules and Seller Penalization Policies](https://sahayakai.co.in/knowledge/order-management/gem-order-cancel-rules)
...
```
[GeM Order Placed] ──> [PRC Generation (Delivery)] ──> [10-Day Return Inspection Window] ──> [CRAC (Payment) OR Rejection (Lifting/Replacement)]
```
...
* GeM General Terms and Conditions (GTC): [https://gem.gov.in/page/detail/34](https://gem.gov.in)
* Department of Expenditure, Procurement Division guidelines: [https://doe.gov.in](https://doe.gov.in)
